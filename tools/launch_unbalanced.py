...
def construct_dgl_server_env_vars(
    ...
    server_env_vars_template = (
        ...
        "DGL_SIMULATE_STRAGGLER={DGL_SIMULATE_STRAGGLER} "
    ...
    return server_env_vars_template.format(
        ...
        DGL_SIMULATE_STRAGGLER=is_straggler,
...
def submit_jobs(args, udf_command, dry_run=False):
    ...
    state_q = queue.Queue()
    num_samplers_list = np.array([args.num_samplers for _ in range(len(hosts))] if args.num_samplers_list is None else args.num_samplers_list)
    straggler_list = [0 for _ in range(len(hosts))] if args.straggler_list is None else args.straggler_list
    tot_num_clients = args.num_trainers * np.sum(1 + num_samplers_list)
    # launch server tasks
    for i in range(len(hosts) * server_count_per_machine):
        server_env_vars = construct_dgl_server_env_vars(
            num_samplers=num_samplers_list[i // args.num_servers],
            ...
            is_straggler=straggler_list[i // args.num_servers]
        )
    ...
    for node_id, host in enumerate(hosts):
        ...
        client_env_vars = construct_dgl_client_env_vars(
            num_samplers=num_samplers_list[node_id],
            tot_num_clients=tot_num_clients,
            part_config=args.part_config,
            ip_config=args.ip_config,
            num_servers=args.num_servers,
            ...
            group_id=0,
            pythonpath=os.environ.get("PYTHONPATH", ""),
            is_straggler=straggler_list[node_id]
        )
...
def main():
    ...
    parser.add_argument(
        ...
        nargs='*',
        type=int,
        help="The number of sampler processes per trainer process in a list",
    )
    parser.add_argument(
        "--straggler_list",
        nargs='*',
        type=int,
        help="Simulate a straggler",
    )
    parser.add_argument(
        "--num_servers",
        type=int,
