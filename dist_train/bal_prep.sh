...
data="data_${GRAPH_NAME}_bal"

if [ ! -d "${data}/" ]; then
  echo "Data does not exist, partition graph first"
  python3 /dgl/examples/distributed/graphsage/partition_graph.py --dataset ${GRAPH_NAME} --num_parts ${NUM_MACHINES} --balance_train --balance_edges
fi
